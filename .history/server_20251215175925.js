...
function getDepartment(topicId) {
  if ([10,11,12,13,14,15,16].includes(topicId)) return "donanim";
  if (topicId === 17) return "kargo";
  if (topicId === 18) return "paketleme";
  if (topicId === 19) return "musteri";
  return "diger";
}
...
topics.forEach(t => {
  const dept = getDepartment(t.topic_id);
  ...
  if (t.risk === "high") deptScore[dept] += 2;
    ...
    // Karar önerisi (en riskli topic)
    const critical = topics.find(t => t.risk === "high");
    let decision = "İzleme önerilir";
    if (critical) {
      if ([10,11,12,14,15,16].includes(critical.topic_id))
        decision = "Donanım iyileştirme önerilir";
      else if (critical.topic_id === 17)
        decision = "Kargo firması değişimi önerilir";
      else if (critical.topic_id === 18)
        decision = "Paketleme iyileştirme önerilir";
      else if (critical.topic_id === 19)
        decision = "Müşteri hizmetleri iyileştirme önerilir";
    }

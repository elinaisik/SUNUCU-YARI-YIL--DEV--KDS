...
// ----------------------------------------------------------------------
// 📌 SİMÜLASYON MOTORU (İŞLETME + ÜRÜN)
// ----------------------------------------------------------------------
app.get("/simulasyon-data", async (req, res) => {
  try {
    const urunID = req.query.urun || "all";
    const conn = await pool.getConnection();
    ...
    // 1️⃣ TOPIC BAZLI ŞİKAYET ORANLARI
    const [topicRows] = await conn.query(`
      SELECT 
        t.topic_id,
        t.topic_adi,
    ...
    const topics = topicRows.map(r => ({
      ...
      oran: Number(((r.adet / total) * 100).toFixed(1))
    }));

    // 2️⃣ EŞİKLER & KARAR
    let decision = "İzleme önerilir";
    let impact = 0;

    const high = topics.find(t => t.oran >= 30);
    if (high) {
      decision = `${high.topic_adi} için iyileştirme önerilir`;
      impact = 0.4;
    }

    // 3️⃣ SİMÜLASYON SONRASI
    const simulated = topics.map(t => ({
      ...t,
      after: impact && t.oran >= 15
        ? Number((t.oran * (1 - impact)).toFixed(1))
        : t.oran
    }));

    // 4️⃣ SUBTOPIC ETKİSİ (SADECE SEÇİLEN TOPIC)
    let subImpact = [];
    if (high) {
      const [subs] = await conn.query(`
        ...
        WHERE st.topic_id = ?
        ${where}
        GROUP BY st.subtopic_id, st.subtopic_adi
        ORDER BY adet DESC
      `, urunID === "all" ? [high.topic_id] : [high.topic_id, ...params]);

      subImpact = subs.map(s => ({
        subtopic_adi: s.subtopic_adi,
        before: s.adet,
        after: Math.round(s.adet * (1 - impact))
      }));
    }
    ...
    res.status(500).json({ error: "Simülasyon hesaplanamadı" });
  }
});
